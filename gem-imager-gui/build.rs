...
fn embed_winusb_helper_hash() {
    ...
    let candidates = [
        target_dir
            .join(&target)
            .join(&profile)
            .join("gem-winusb-helper.exe"),
        target_dir.join(&profile).join("gem-winusb-helper.exe"),
    ];
    ...
    let hash = match helper {
        ...
        None if profile == "release" && target.contains("windows") => panic!(
            "dfu-driver-mvp release builds require gem-winusb-helper.exe first; use the Windows packaging target"
        ),
        None => String::new(),
    };
    println!("cargo:rustc-env=GEM_WINUSB_HELPER_SHA256={hash}");
}
