...
pub(crate) fn view<'a>(state: &'a crate::state::ChooseOsState) -> Element<'a, GemImagerMessage> {
...
fn os_list_pane<'a>(state: &'a crate::state::ChooseOsState) -> Element<'a, GemImagerMessage> {
    if state.images.is_empty() {
        ...
        let items = state
            ...
            .map(|img| {
                ...
                let icon: Element<GemImagerMessage> = match img.id {
                    ...
                    | crate::helpers::OsImageId::OsSublist(_) => gem_iced_widgets::cached_icon(
                ...
                helpers::list_item(contents, is_selected, GemImagerMessage::SelectOs(img.id))
            })
            .map(Into::into);
        ...
        let back: Vec<Element<GemImagerMessage>> = if state.pos.is_none() {
            ...
            vec![
                helpers::list_item(
                    ...
                    false,
                    GemImagerMessage::GotoOsListParent,
                )
                .into(),
            ]
        };
...
fn os_view_pane<'a>(state: &'a crate::state::ChooseOsState) -> Element<'a, GemImagerMessage> {
    match state.selected_image.as_ref() {
        Some((id, img)) => {
            let icon: Element<'a, GemImagerMessage> = match img.icon() {
                crate::helpers::BoardImageIcon::Remote(url) => {
                    gem_iced_widgets::cached_icon(&state.common.img_handle_cache, url)
            ...
            if let Some(json) = state.img_json() {
                col = col.push(widget::center(
                    helpers::copy_btn(helpers::COPY_ICON.clone())
                        .on_press(GemImagerMessage::CopyToClipboard(json)),
                ));
            }
            ...
            if init_formats.len() > 1 {
                ...
                let el = widget::pick_list(
                    init_formats,
                    if init_format == gem_config::config::InitFormat::None {
                        None
                    } else {
                        Some(init_format)
                    },
                    GemImagerMessage::UpdateInitFormat,
                );
                col = col.push(
                    widget::row![
