...
use gem_flasher::t3_gem_init::{Hostname, Secret, Ssid, WifiCountry};
use gem_i18n::Msg;
...
pub(crate) fn view<'a>(state: &'a crate::state::CustomizeState) -> Element<'a, GemImagerMessage> {
    ...
    page_type2(
        ...
        [
            widget::button(lang.text(Msg::Reset))
                ...
                .on_press(GemImagerMessage::ResetFlashingConfig),
...
fn t3_gem_init<'a>(
    ...
    col = col.push(
        widget::toggler(config.user_password.is_some())
            ...
            .on_toggle(move |t| {
                ...
                GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_user_password(c)))
            }),
    );
    ...
    let toggle = widget::toggler(config.timezone.is_some())
        ...
        .on_toggle(move |t| {
            ...
            GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_timezone(tz)))
        });
    ...
    let toggle = widget::toggler(config.hostname.is_some())
        ...
        .on_toggle(move |t| {
            ...
            GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_hostname(hostname)))
        });
    ...
    let toggle = widget::toggler(config.keymap.is_some())
        ...
        .on_toggle(move |t| {
            ...
            GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_keymap(keymap)))
        });
    ...
    if desktop {
        ...
        col = col.push(
            widget::toggler(config.vnc.is_some())
                ...
                .on_toggle(move |t| {
                    let c = if t { Some(Default::default()) } else { None };
                    GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_vnc(c)))
                }),
        );
    ...
    F: 'a + Fn(String) -> FlashingCustomization,
{
    element_with_label(
        ...
        widget::text_input(placeholder, value.as_input())
            .secure(true)
            .on_input(move |inp| GemImagerMessage::UpdateFlashConfig(update_config_cb(inp)))
            .width(INPUT_WIDTH)
            .into(),
    )
}
...
fn hint(message: &str) -> Element<'_, GemImagerMessage> {
    widget::container(text(message.to_owned()).size(12))
        .padding(iced::Padding::ZERO.horizontal(16))
        .into()
}

fn error_text(message: String) -> Element<'static, GemImagerMessage> {
    widget::container(
        text(message)
            .size(12)
...
fn linux_sd_card_common<'a>(
...
) -> widget::Column<'a, GemImagerMessage> {
    let lang = state.common.lang();
    let mut col = widget::column([]);
    ...
    col = col.push(
        widget::toggler(config.user.is_some())
            ...
            .on_toggle(move |t| {
                let c = if t { Some(Default::default()) } else { None };
                GemImagerMessage::UpdateFlashConfig(wrap(config.clone().update_user(c)))
            }),
    );
...
fn linux_sd_card_sysconfig<'a>(
    ...
    col = col.push(
        widget::toggler(config.usb_enable_dhcp == Some(true))
            ...
            .on_toggle(|x| {
                GemImagerMessage::UpdateFlashConfig(FlashingCustomization::LinuxSdSysconfig(
