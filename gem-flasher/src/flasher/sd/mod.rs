...
const fn translate_status(
    ...
    match status {
        gem_flasher_sd::Status::Preparing => DownloadFlashingStatus::Preparing,
        gem_flasher_sd::Status::Writing(x) => {
            if is_file_dest {
        ...
        gem_flasher_sd::Status::Verifying(x) => DownloadFlashingStatus::Verifying(x),
        gem_flasher_sd::Status::Customizing => DownloadFlashingStatus::Customizing,
    }
}
...
mod status_tests {
    ...
    fn writing_to_a_device_is_flashing_and_to_a_file_is_downloading() {
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Writing(0.5), false),
            DownloadFlashingStatus::FlashingProgress(0.5)
        );
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Writing(0.5), true),
    ...
    fn verification_keeps_its_own_stage_and_fraction() {
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Verifying(0.25), false),
            DownloadFlashingStatus::Verifying(0.25)
        );
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Verifying(0.25), true),
            DownloadFlashingStatus::Verifying(0.25)
        );
    }
    ...
    fn preparing_and_customizing_pass_through() {
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Preparing, false),
            DownloadFlashingStatus::Preparing
        );
        assert_eq!(
            translate_status(gem_flasher_sd::Status::Customizing, false),
...
pub struct Flasher<I> {
    img: I,
    dst: gem_flasher_sd::Destination,
...
impl<I> Flasher<I> {
    ...
    pub fn with_file_dest(img: I, dst: PathBuf, customization: FlashingSdLinuxConfig) -> Self {
        Self {
            img,
            dst: gem_flasher_sd::Destination::File(dst.into_boxed_path()),
            customization,
        }
    }

    const fn is_file_dest(&self) -> bool {
        matches!(self.dst, gem_flasher_sd::Destination::File(_))
    }
}
    ...
    I: FnOnce() -> std::io::Result<(crate::img::OsImage, u64)> + Send,
{
    pub fn flash(
        ...
        let customization = if self.customization.0.is_empty() {
            ...
            vec![gem_flasher_sd::Customization {
                partition: gem_flasher_sd::ParitionType::Boot,
                content,
            }]
        }
        .into_iter();
        ...
        gem_flasher_sd::flash(self.img, self.dst, tx, customization, cancel).map_err(Into::into)
    }
}
