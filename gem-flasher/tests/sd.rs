...
use gem_flasher::{
    DownloadFlashingStatus, GemFlasherTarget, img::OsImage, sd::FlashingSdLinuxConfig,
};
...
fn mock_img() -> OsImage {
    ...
    OsImage::from_path(f.path(), gem_flasher::img::ExtractGate::LocalFile).unwrap()
}
...
fn flash_cancel() {
    ...
    let res = gem_flasher::sd::Flasher::with_file_dest(
        || Ok((mock_img(), MOCK_IMG_LEN as u64)),
        sd.path().to_path_buf(),
        FlashingSdLinuxConfig::none(),
    )
...
#[test]
fn destinations() {
    let temp = gem_flasher::sd::Target::destinations(false);
