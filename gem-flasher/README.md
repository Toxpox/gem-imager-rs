...
use gem_flasher::BBFlasher;

#[tokio::main]
async fn main() {
    let img = gem_flasher::LocalImage::new("/tmp/abc.img.xz".into());
    let target = PathBuf::from("/tmp/target").try_into().unwrap();
    let customization = 
        gem_flasher::sd::FlashingSdLinuxConfig::new(None, None, None, None, None);

    let flasher = gem_flasher::sd::Flasher::new(img, target, customization)
        .flash(None)
        .await
        .unwrap();
}
