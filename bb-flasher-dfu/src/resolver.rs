...
use bb_config::t3::{DfuProfile, VerifiedBootManifest, parse_boot_manifest};
use bb_downloader::{ArchiveIntegrity, Downloader};
use bb_helper::{cancel::CancellationToken, file_stream::file_stream};
...
mod tests {
    use super::*;
    use bb_config::t3::DfuProfile;
    use bb_downloader::TransportPolicy;
    ...
    async fn artifact_hash_mismatch_stops_before_usb() {
        ...
        assert!(matches!(
            ...
            Err(Error::BootArtifactDownload(
                bb_downloader::DownloadError::ArchiveHashMismatch { .. }
            ))
        ));
    }
}
