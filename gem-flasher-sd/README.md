...
Library to flash SD cards with OS images. Powers SD card flashing in [T3 Gemstone Imager](https://github.com/Toxpox/gem-imager-rs).
...
async fn main() {
    ...
    let flash_thread = tokio::spawn(async move { gem_flasher_sd::flash(img, None::<std::future::Ready<std::io::Result<Box<str>>>>, dst, Some(tx), Vec::new(), None).await });
