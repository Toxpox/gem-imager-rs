...
$gui = Join-Path $release 'bb-imager-gui.exe'
$helper = Join-Path $release 'bb-winusb-helper.exe'
$runtime = Join-Path $repoRoot 'bb-winusb\native\x86_64-pc-windows-msvc\libwdi.dll'
...
$dist = Join-Path $repoRoot 'bb-imager-gui\dist'
...
Copy-Item -LiteralPath $gui -Destination (Join-Path $bundle 'bb-imager-gui.exe')
Copy-Item -LiteralPath $helper -Destination (Join-Path $bundle 'bb-winusb-helper.exe')
Copy-Item -LiteralPath $runtime -Destination (Join-Path $bundle 'libwdi.dll')
Copy-Item -LiteralPath (Join-Path $repoRoot 'bb-winusb\third_party\libwdi\COPYING-LGPL') -Destination $bundle
Copy-Item -LiteralPath (Join-Path $repoRoot 'bb-winusb\third_party\libwdi\Microsoft-WDF-License.rtf') -Destination $bundle
